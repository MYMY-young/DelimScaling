    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            model_inputs.update({"select_layer": model_kwargs["select_layer"]})
            model_inputs.update({"delim_scaling": model_kwargs["delim_scaling"]})
            model_inputs.update({"scale": model_kwargs["scale"]})
            if is_prefill:
